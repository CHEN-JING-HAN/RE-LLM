...
feature_schema = Features({
    ...
    "w2v2_instruction_ids": Sequence(Value("int64")),  # Changed from null to int64
    "w2v2_instruction_mask": Sequence(Value("int64")),  # Changed from null to int64
    "w2v2_instruction_labels": Sequence(Value("int64")),  # Changed from null to int64
    ...
    "arousal_ans":Value("float"),
    "valence_ans":Value("float"),
    "dominance_ans":Value("float"),
    "to_keep": Value("bool"),
...
def process_dataset(
    ...
    w2v2_instruction = "Consider the following emotion.\n emotion: "
    w2v2_instruction_ids = _tokenize_str(content=w2v2_instruction)
    w2v2_instruction_mask = [1] * len(w2v2_instruction_ids)
    w2v2_instruction_labels = [-100] * len(w2v2_instruction_ids)
    ...
    arousal_ans = batch["avd_ans"]["arousal"]
    valence_ans = batch["avd_ans"]["valence"]
    dominance_ans = batch["avd_ans"]["dominance"]
    batch["arousal_ans"] = arousal_ans
    batch["valence_ans"] = valence_ans
    batch["dominance_ans"] = dominance_ans
    ...
    batch["w2v2_instruction_ids"] = w2v2_instruction_ids
    batch["w2v2_instruction_mask"] = w2v2_instruction_mask
    batch["w2v2_instruction_labels"] = w2v2_instruction_labels

    print("instruction_dataset_process: in 178")
    print(f"audio_path:{batch}\n") #aro:{batch["arousal_ans"]}, val:{batch["valence_ans"]}, dom:{batch["dominance_ans"]}")

    batch["to_keep"] = to_keep
    if audio_path:
        batch["audio_path"] = audio_path
    #print(f"w2v2_instr.ids:{w2v2_instruction_ids}\nw2v2_instr.decoded_text:{tokenizer._decode(w2v2_instruction_ids, skip_special_tokens=True)}")
    #print(f"in process_dataset batch : \n{batch}")
    return batch
...
class InstructionDataCollator:
    ...
    def __call__(self, samples: List[Dict]):
        ...
        arousal_ans = [sample["arousal_ans"] for sample in samples]
        valence_ans = [sample["valence_ans"] for sample in samples]
        dominance_ans = [sample["dominance_ans"] for sample in samples]
        w2v2_instruction_ids = [sample["w2v2_instruction_ids"] for sample in samples]
        w2v2_instruction_mask = [sample["w2v2_instruction_mask"] for sample in samples]
        w2v2_instruction_labels = [sample["w2v2_instruction_labels"] for sample in samples]
        ...
        arousal_ans = torch.FloatTensor(arousal_ans)
        valence_ans = torch.FloatTensor(valence_ans)
        dominance_ans = torch.FloatTensor(dominance_ans)
        w2v2_instruction_ids = collate_tokens(w2v2_instruction_ids, self.pad_id)
        w2v2_instruction_mask = collate_tokens(w2v2_instruction_mask, 0)
        w2v2_instruction_labels = collate_tokens(w2v2_instruction_labels, -100)
        #print(f"In In class InstructionDataCollator: \n w2v2instrid{w2v2_instruction_ids}\n w2v2instrmk{w2v2_instruction_mask}\n w2v2instrlabels{w2v2_instruction_labels}")
        ...
        if all(len(sample) == 0 for sample in raw_speech):
            ...
            #print(f"In class InstructionDataCollator: \nraw speech:--size:{len(raw_speech)}--type{raw_speech}\n{raw_speech} \nspeech values:--size:{len(speech_values)}--type{speech_values}\n{speech_values}\nspeech mask:--size:{len(speech_mask)}--type{speech_mask}\n{speech_mask}")
        ...
        return {
            ...
            "arousal_ans": arousal_ans,
            "valence_ans": valence_ans,
            "dominance_ans": dominance_ans,
            "speech_values": speech_values,
            "speech_mask": speech_mask,
            "raw_speech":raw_speech,
            "sampling_rate":self.sampling_rate,
            "w2v2_instruction_ids": w2v2_instruction_ids,
            "w2v2_instruction_mask": w2v2_instruction_mask,
            "w2v2_instruction_labels": w2v2_instruction_labels,
        }
...
def offline_process(
    ...
    for key in dataset[0].keys():
        if key != "audio_path" and key != "to_keep" and key != "emotion_labels" and key != "avd_ans" and key != "arousal_ans" and key != "valence_ans" and key != "dominance_ans":
