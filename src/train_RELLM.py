...
from src.instruction_dataset_w2v2cat import load_instruction_datasets, InstructionDataCollator
...
class ModelArguments:
    ...
    unfreeze_speechSERclr: bool = field(
        default=False, metadata={"help": "whether to unfreeze the speech_SER_classifier parameters"}
    )
    unfreeze_w2v2_adapter: bool = field(
        default=False, metadata={"help": "whether to unfreeze the w2v2_adapter parameters"}
    )
    unfreeze_w2v2SERclr: bool = field(
        default=False, metadata={"help": "whether to unfreeze the w2v2_SER_classifier parameters"}
    )
    unfreeze_w2v2: bool = field(
        default=False, metadata={"help": "whether to unfreeze the w2v2 parameters"}
    )
...
def main():
    ...
    if not model_args.unfreeze_speechSERclr:
        for name, param in model.hidden2emotion.named_parameters():
            param.requires_grad = False
    
    if not model_args.unfreeze_w2v2_adapter:
        for name, param in model.w2v2_adapter.named_parameters():
            param.requires_grad = False

    if not model_args.unfreeze_w2v2SERclr:
        for name, param in model.w2v2_hidden2emotion.named_parameters():
            param.requires_grad = False
    if not model_args.unfreeze_w2v2:
        for name, param in model.w2v2.named_parameters():
            param.requires_grad = False
