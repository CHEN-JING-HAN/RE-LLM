...
from src.modeling_blsp2_w2v2cat import Blsp2Model
...
class DataCollator:
    ...
    def __call__(self, samples: List[Dict]):
        ...
        w2v2_sep_ids = [sample["w2v2_sep_ids"] for sample in samples]
        w2v2_sep_mask = [sample["w2v2_sep_mask"] for sample in samples]
        attention_mask = [sample["attention_mask"] for sample in samples]
        ...
        w2v2_sep_ids = collate_tokens(w2v2_sep_ids, self.pad_id)
        w2v2_sep_mask = collate_tokens(w2v2_sep_mask, 0)
        ...
        if all(len(sample) == 0 for sample in raw_speech):
            ...
            speech_values = speech_inputs.input_features.to(torch.float32)
        ...
        return {
            ...
            "sampling_rate":self.sampling_rate,
            "w2v2_sep_ids":w2v2_sep_ids,
            "w2v2_sep_mask":w2v2_sep_mask,
            "reference": reference,
            "audio":audio,
            "text":text,
        }
...
def main():
    ...
    def process_dataset(batch):
        ...
        if args.audio_field:
            ...
            batch["audio"] = None
        ### w2v2 seperate ids
        w2v2_seperate_prompt = "Consider the following emotional embedding. "
        w2v2_sep_ids =  _tokenize_str("user", w2v2_seperate_prompt)
        batch["w2v2_sep_ids"] = w2v2_sep_ids
        batch["w2v2_sep_mask"] = [1] * len(batch["w2v2_sep_ids"])
        ### suffix
        ...
        if args.reference_field:
            ...
            batch["reference"] = ""
        batch["reference"] = batch.get("emotion", "")
        batch["text"] = batch.get("text", "")

        return batch
    
    dataset = dataset.map(process_dataset)
    model = Blsp2Model.from_pretrained(args.blsp_model, torch_dtype=torch.float32)
    ...
    with open(args.output_file, "w") as fout:
        for batch in tqdm(dataloader):
            #item = batch["raw_speech"]
            #print(f"In generate_w2v2.py print batch:\n{item}")
            #raw_speech_t = torch.FloatTensor(batch["raw_speech"])
            outputs = model.generate(
                input_ids=batch["input_ids"].cuda(),
                attention_mask=batch["attention_mask"].cuda(),
                suffix_input_ids=batch["suffix_input_ids"].cuda(),
                suffix_attention_mask=batch["suffix_attention_mask"].cuda(),
                raw_speech=batch["raw_speech"], #batch in numpy format with well-design w2v2 process function
                sampling_rate = batch["sampling_rate"],
                reference=batch["reference"],
                w2v2_sep_ids = batch["w2v2_sep_ids"].cuda(),
                w2v2_sep_mask= batch["w2v2_sep_mask"].cuda(),
            ...
            '''
            #print(f"in fenerate_w2v2.py 289:\nw2v2emb_text:{w2v2emb_text}")
            output_text = [
                decode_tokens(
                    output,
                    tokenizer,
            ...
            for reference, response, audio_path, transcript in zip(batch["reference"], output_text, batch["audio"], batch["text"]):
